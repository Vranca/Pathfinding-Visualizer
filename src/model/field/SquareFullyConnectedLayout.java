...
public class SquareFullyConnectedLayout implements FieldLayout
{
	...
	public ArrayList<Cell> setLayout(int rowCount, int columnCount)
	{
		...
		for (int i = 0; i < (rowCount * columnCount); i++)
		{
			...
			if (i < ((rowCount - 1) * columnCount) && (i % columnCount) != columnCount - 1)
			{
				field.get(i).addNeighbour(field.get(i + (columnCount + 1)), 1.41);
			}
			// Not top or left edge
			if (i > columnCount - 1 && (i % columnCount) != 0)
			{
				field.get(i).addNeighbour(field.get(i - (columnCount + 1)), 1.41);
			}
			// Not bottom or left edge
			if (i < ((rowCount - 1) * columnCount) && (i % columnCount) != 0)
			{
				field.get(i).addNeighbour(field.get(i + (columnCount - 1)), 1.41);
			}
			// Not top or right edge
